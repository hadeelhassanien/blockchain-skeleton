...
class Blockchain:
    ...
    def __init__(self):
        ...
        """
        Create the gensis block, and append it to the chain. The block has index 0, previous_hash as 0, and
        a valid hash.
        """
    ...
    def add_block(self, block, proof): 
        ...
        if not self.is_valid_proof(block, proof):   
            print("there was an error with proof of work")
            return False
        if not self.last_block.hash == block.previous_hash: 
            print("previous hash and the hash of the last block do not match")
            return False 
    ...
    def proof_of_work(self, block):
        """
        Implement the basic proof of work. Specifically, update the nonce value from its default of 0 until the hash of the block begins with the defined number of 0s,
        """
        block.nonce = 0
        blockHash = block.compute_hash()
        while not blockHash.startswith('0' * Blockchain.difficulty):
            block.nonce += 1 
            blockHash = block.compute_hash() #recompute each time nonce is incremented
        return blockHash #the proof is supposed to be a valid hash
...
if __name__ == "__main__":
    ...
    if not len(blockchain.chain) == 1:
        print("Genesis block was not successfully created")
    else:
        print("Genesis block was successfully created!")
    ...
    blockchain.add_new_transaction("Hadeel pays Basel 100 dollars")
    blockchain.add_new_transaction("Basel pays Hadeel 60 dollars") 
    blockchain.add_new_transaction("Omar pays Hadeel 5 dollars")
    blockchain.add_new_transaction("Hadeel pays Omar 16 dollars")
    blockchain.add_new_transaction("Basel pays Omar 77 dollars") 

    print("5 transactions added \n")


    exampleIndex = blockchain.mine()

    if len(blockchain.chain) == 2:
        print("Mining of a block was successful!")
    else: 
        print("Mining of a block was not successful.")

    print("index of mined block: ", exampleIndex)
